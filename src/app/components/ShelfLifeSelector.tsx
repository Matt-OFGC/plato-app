...
export function ShelfLifeSelector({ 
  ...
  const handleCreateOption = async () => {
    if (!newOptionName.trim()) {
      alert("Please enter a shelf life option name");
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success && data.option) {
        ...
        const errorMsg = data.error || data.details?.[0]?.message || "Failed to create shelf life option";
        alert(`Error: ${errorMsg}`);
        setIsCreating(false);
      }
    } catch (error) {
      console.error("Error creating shelf life option:", error);
      alert(`Failed to create shelf life option: ${error instanceof Error ? error.message : "Unknown error"}`);
      setIsCreating(false);
    }
  };
                  ...
                  <div className="border-t border-gray-200">
                    {!showCreateForm ? (
                      <button
                        type="button"
                        onClick={() => {
                          const nameToUse = searchTerm.trim() || "New Shelf Life Option";
                      ...
                      <div className="border-t border-gray-200 mt-2 pt-2">
                        <div className="px-2 py-2">
                          <div className="text-xs font-medium text-gray-700 mb-2">Create new shelf life option</div>
                          <input
                            type="text"
                            value={newOptionName}
                            ...
                            placeholder="Shelf life option name"
