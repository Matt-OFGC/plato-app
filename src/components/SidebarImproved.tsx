...
export function Sidebar() {
  ...
  // Main nav items for bottom nav (most used)
  const bottomNavItems = [
    navItems[0], // Dashboard
    navItems[1], // Ingredients  
    navItems[2], // Recipes
    navItems[3], // Production
    { 
      href: "#more",
      label: "More",
      shortLabel: "More",
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 safe-area-inset">
        <div className="grid grid-cols-5 h-16">
          {bottomNavItems.map((item) => {
            const active = item.href === "#more" ? false : isActive(item.href) || isPathStartsWith(item.href);
            return (
              <Link
                key={item.href}
                href={item.href === "#more" ? pathname : item.href}
                onClick={(e) => {
                  if (item.onClick) {
                    e.preventDefault();
                    item.onClick();
      ...
      {/* TABLET SIDEBAR (iPad) - Slim persistent */}
      <aside className="hidden md:flex lg:hidden fixed left-0 top-0 h-full bg-white border-r border-gray-200 z-30 w-20 flex-col">
        {/* Company Logo */}
        <div className="p-3 border-b border-gray-200">
          <Link href="/dashboard" className="flex justify-center">
            {company?.logoUrl ? (
              ...
              <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-white text-xl font-bold">
                P
              </div>
            )}
          </Link>
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center gap-1 py-3 transition-colors ${
                  active ? "text-green-600" : "text-gray-600 hover:text-gray-900 hover:bg-gray-50"
                }`}
              >
                {item.icon}
